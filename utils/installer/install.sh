...
declare -r NIL_LOG_LEVEL="${NIL_LOG_LEVEL:-warn}"
...
function usage() {
  ...
  echo "    -l, --local                              Install local copy of nil"
  echo "    -y, --yes                                Disable confirmation prompts (answer yes to all questions)"
  echo "    --overwrite                              Overwrite previous nil configuration (a backup is always performed first)"
...
function main() {
  ...
  if [ "$ARGS_INSTALL_DEPENDENCIES" -eq 1 ]; then
    if [ "$INTERACTIVE_MODE" -eq 1 ]; then
      if confirm "Would you like to install nil's NodeJS dependencies?"; then
        install_nodejs_deps
      fi
      if confirm "Would you like to install nil's Python dependencies?"; then
        install_python_deps
      fi
      if confirm "Would you like to install nil's Rust dependencies?"; then
        install_rust_deps
      fi
    else
      install_nodejs_deps
  ...
  echo "You can start it by running: $INSTALL_PREFIX/bin/nil"
...
function check_neovim_min_version() {
  ...
  if ! nvim --headless -u NONE -c "$verify_version_cmd"; then
    echo "[ERROR]: nil requires at least Neovim v0.7 or higher"
    exit 1
  fi
}
...
function clone_nil() {
  msg "Cloning nil configuration"
  git clone https://github.com/nil-lua/nil.git ~/.local/share/nil/nil/
}

function link_local_nil() {
  echo "Linking local nil repo"

  # Detect whether it's a symlink or a folder
  if [ -d "$NIL_BASE_DIR" ]; then
    echo "Removing old installation files"
    rm -rf "$NIL_BASE_DIR"
  fi

  echo "   - $BASEDIR -> $NIL_BASE_DIR"
  ln -s -f "$BASEDIR" "$NIL_BASE_DIR"
}

function setup_shim() {
  make -C "$NIL_BASE_DIR" install-bin
}

function remove_old_cache_files() {
  local packer_cache="$NIL_CONFIG_DIR/plugin/packer_compiled.lua"
  if [ -e "$packer_cache" ]; then
    msg "Removing old packer cache file"
    rm -f "$packer_cache"
  fi

  if [ -e "$NIL_CACHE_DIR/luacache" ] || [ -e "$NIL_CACHE_DIR/nil_cache" ]; then
    msg "Removing old startup cache file"
    rm -f "$NIL_CACHE_DIR/{luacache,nil_cache}"
  fi
}
...
function setup_nil() {
  ...
  cp "$NIL_BASE_DIR/utils/installer/config.example.lua" "$NIL_CONFIG_DIR/config.lua"

  echo "Preparing Packer setup"

  "$INSTALL_PREFIX/bin/nil" --headless \
    -c "lua require('nil.core.log'):set_level([[$NIL_LOG_LEVEL]])" \
