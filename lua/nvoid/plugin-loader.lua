...
local default_snapshot = join_paths(get_nil_base_dir(), "snapshots", "default.json")
...
function plugin_loader.init(opts)
  ...
  if status_ok then
    packer.on_complete = vim.schedule_wrap(function()
      require("nil.utils.hooks").run_on_packer_complete()
    end)
...
function plugin_loader.load(configurations)
  ...
  local status_ok, _ = xpcall(function()
    ...
    vim.g.theme = nil.colorscheme
    vim.g.transparency = nil.transparent_window
  end, debug.traceback)
  if not status_ok then
...
function plugin_loader.load_snapshot(snapshot_file)
  ...
  if not in_headless then
    vim.notify("Syncing core plugins is in progress..", vim.log.levels.INFO, { title = "nil" })
  end
...
function plugin_loader.sync_core_plugins()
  ...
  vim.api.nvim_create_autocmd("User", {
    ...
    once = true,
    callback = function()
      require("nil.plugin-loader").load_snapshot(default_snapshot)
    end,
  })
