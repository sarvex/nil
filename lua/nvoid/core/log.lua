...
function Log:set_level(level)
  local logger_ok, _ = xpcall(function()
    ...
    if structlog then
      local logger = structlog.get_logger "nil"
  ...
  local packer_ok, _ = xpcall(function()
    ...
    require("packer.log").new { level = nil.log.level }
  end, debug.traceback)
...
function Log:init()
  ...
  local log_level = Log.levels[(nil.log.level):upper() or "WARN"]
  local nil_log = {
    nil = {
      sinks = {
  ...
  structlog.configure(nil_log)
  local logger = structlog.get_logger "nil"

  -- Overwrite `vim.notify` to use the logger
  if nil.log.override_notify then
...
function Log:get_path()
  return string.format("%s/%s.log", get_cache_dir(), "nil")
end
