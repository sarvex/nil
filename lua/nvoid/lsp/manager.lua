...
local function resolve_config(server_name, ...)
  local defaults = {
    on_attach = require("nil.lsp").common_on_attach,
    on_init = require("nil.lsp").common_on_init,
    on_exit = require("nil.lsp").common_on_exit,
    capabilities = require("nil.lsp").common_capabilities(),
  }

  local has_custom_provider, custom_config = pcall(require, "nil/lsp/providers/" .. server_name)
...
function M.setup(server_name, user_config)
  ...
  if nil_lsp_utils.is_client_active(server_name) or client_is_configured(server_name) then
    return
  end
  ...
  local should_auto_install = function(name)
    local installer_settings = nil.lsp.installer.setup
