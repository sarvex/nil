...
local function create_floating_file(location, opts)
  ...
  local contents = vim.api.nvim_buf_get_lines(
    ...
    range.start.line,
    math.min(
      range["end"].line + 1 + (opts.context or nil.lsp.peek.max_height),
      range.start.line + (opts.max_height or nil.lsp.peek.max_height)
    ),
    false
  )
  ...
  opts = vim.lsp.util.make_floating_popup_options(
    if_nil(width, nil.lsp.peek.max_width),
    if_nil(height, nil.lsp.peek.max_height),
    opts
  )
...
function M.Peek(what)
  ...
  if vim.tbl_contains(vim.api.nvim_list_wins(), M.floating_win) then
    ...
    vim.api.nvim_buf_set_keymap(
      ...
      "n",
      "<CR>",
      ":lua require('nil.lsp.peek').open_file()<CR>",
