...
local fn = vim.fn
local plugins_count = fn.len(fn.globpath("~/.local/share/nil/site/pack/packer/start/", "*", 0, 1))
...
local buttons = {
  ...
  val = {
    ...
    button("e", "  Configuration", ":e ~/.config/nil/config.lua <CR>"),
    button("u", "  Update plugins", ":PackerSync<CR>"),
    button("U", "ﮮ  Update Nil", ":NilUpdater<CR>"),
    button("q", "  Quit", ":qa<CR>"),
  },
  opts = {
    spacing = 0,
...
local heading = {
  type = "text",
  val = text.align_center({ width = 0 }, { "Nil loaded " .. plugins_count .. " plugins  " }, 0.5),
  opts = {
    position = "center",
...
local nil_site = "爵nil.org"
local footer = {
  type = "text",
  val = text.align_center({ width = 0 }, { nil_site }, 0.5),
  opts = {
    position = "center",
    hl = "DashboardFooter",
  },
